...
void zero(u_int8_t *str, u_int16_t size){
    u_int8_t *p;
    u_int16_t n;
...
void childLoop(Client *cli){
    sleep(1);
    dprintf(cli->s,"Kenneth says hello!\n");
...
void mainLoop(int s){
    ...
    u_int32_t len; // Will be used to hold the length of the address that is returned by accept()
    char *ip;
    u_int16_t port;
    ...
    // Return the port 
    port = (u_int16_t)htons((int)cli.sin_port);
    // Return the address
    ...
    zero((u_int8_t *)client, sizeof(struct s_client));
